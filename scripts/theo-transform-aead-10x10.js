...
const source = '/rRoSapsG0mYJtfMxKA3LigccFOylL+ZL7stK8x1dk+43Z2sjXhINL+q1BtWBSCQBfnAJXRwYkBNGBxZyinKV+Iz3vSpfRLa6kj=';
const seedBases = [
...
const transformed = [];
let grid = mkGrid(source, 10, 10);
for (let rot = 0; rot < 4; rot++) {
  for (const [routeName, routeFn] of [['rows', rows], ['cols', cols], ['boustroRows', boustroRows], ['spiral', spiral]]) {
    ...
    for (const [postName, postFn] of [
      ...
      if (decoded && decoded.length === 74) {
        transformed.push({
          desc: `10x10:rot${rot * 90}:${routeName}:${postName}`,
          decoded,
        });
      }
    }
